...
NEW_METH = (("Fieller-JV", "FJV"), ("MOVER-R", "MR"), ("positivity", "Pos"))
for meth, sh in NEW_METH:
    b = r2["adaptive"]["methods"][meth]
    m("AKindLn" + sh, f3(b["kinds"]["line"])); m("ARep" + sh, f3(b["nondeg"]["report"][0]))
    m("ACond" + sh, f3(b["nondeg"]["conditional"][0])); m("AJoint" + sh, f3(b["nondeg"]["joint"][0]))
    m("AWid" + sh, f3(b["nondeg"]["width_median"][0])); m("ASetCov" + sh, f3(b["set_coverage"][0]))
    m("AISRep" + sh, f3(b["score"]["nondeg"]["finite_IS_mean"]))
    for lam, lt in LAMT.items():
        m("ASNd" + lt + sh, f3(b["score"]["nondeg"][lam][0]))
for meth, sh in METH:
    m("AISRep" + sh, f3(r2["adaptive"]["methods"][meth]["score"]["nondeg"]["finite_IS_mean"]))
...
from scipy.stats import t as _tdist, beta as _beta
import numpy as np
m("CVTwoClosed", "%.3f" % (1.0 / float(_tdist.ppf(1.0 - ALPHA_LEVEL / 2.0, 1))))
for nm, tag in (("Fieller", "F"), ("raw delta", "D"), ("delta-JV", "JV"), ("Jeffreys+cost", "JC"),
                ("Bayes", "By"), ("conf. seq.", "Cs")):
    m("Score" + tag, f3(A["methods"][nm]["score"]["nondeg"]["0.50"][0]))

# ------------------------------------------------------------------ frozen-holdout diagnostics (analysis_r8_scale.py)
r8 = J("r8_scale.json")
fr = r8["fieller_reports"]
m("GnMed", "%d" % fr["n_quartiles"][1]); m("GnQlo", "%d" % fr["n_quartiles"][0]); m("GnQhi", "%d" % fr["n_quartiles"][2])
m("GCut", "%.1f" % r8["g_cut"]); m("GShareLowPct", "%.0f\\%%" % (100 * fr["share_g_below_cut"]))
m("GShareHighPct", "%.0f\\%%" % (100 * (1 - fr["share_g_below_cut"])))
m("GCovF", f3(fr["fieller_cov_g_above"])); m("GCovD", f3(fr["delta_cov_g_above"]))
assert r8["mover_r_lead"]["contiguous"]
m("LamLo", "%.2f" % r8["mover_r_lead"]["lo"]); m("LamHi", "%.2f" % r8["mover_r_lead"]["hi"])
m("LamLoRho", "%.0f" % (r8["mover_r_lead"]["lo"] / r8["rho_max"])); m("LamHiRho", "%.0f" % (r8["mover_r_lead"]["hi"] / r8["rho_max"]))
for nm, tag in (("MOVER-R", "MR"), ("Bayes", "By")):
    w = r8["widths"][nm]
    m("WMean" + tag, f3(w["mean"])); m("WPninety" + tag, f3(w["p90"]))
_adapt = [c for c in x4["cells"] if "2.91" in c["cost_dist"] and c["mechanism"] != "uniform"]
m("XfHiCvGap", f3(max(abs(c["conditional"][0] - c["delta_on_F"][0]) for c in _adapt)))
m("XfUnifCondMed", "%.2f" % float(np.median([c["conditional"][0] for c in x4["cells"] if c["mechanism"] == "uniform"])))
m("RealMedRateBexact", "%.4f" % src["e8"]["med"])
m("JeffLoTwo", "%.1f\\times10^{-4}" % (1e4 * float(_beta.ppf(ALPHA_LEVEL / 4.0, 0.5, 2.5))))
# ------------------------------------------------------------------ outcome-correlated costs (analysis_r9_correlated.py)
r9 = J("r9_correlated.json")
_g = [r9["gamma=%g" % g] for g in r9["gammas"]]
def _sgn2(x):
    return ("$-$%.2f" % -x) if x < 0 else "%.2f" % x
_gl = [_sgn2(g).replace("0.50", "0.5").replace("1.00", "1") for g in r9["gammas"]]
m("CorrGammaList", ", ".join(_gl[:-1]) + " and " + _gl[-1])
m("CorrRcLo", _sgn2(min(x["corr_rc_range"][0] for x in _g))); m("CorrRcHi", _sgn2(max(x["corr_rc_range"][1] for x in _g)))
_cf = [x["methods"]["Fieller"]["nondeg"]["conditional"][0] for x in _g]
m("CorrCondFLo", f3(min(_cf))); m("CorrCondFHi", f3(max(_cf)))
_jv = [next(t for t in x["confirmatory_tests"] if t["metric"].startswith("S_") and t["B"] == "delta-JV") for x in _g]
assert all(t["difference"][2] < 0 for t in _jv)
m("CorrScJVLo", f3(min(-t["difference"][0] for t in _jv))); m("CorrScJVHi", f3(max(-t["difference"][0] for t in _jv)))
m("CorrHolmScJVMax", pfmt(max(t["holm_p"] for t in _jv)))
for x in _g:
    _s = {k: v["score"]["nondeg"]["0.50"][0] for k, v in x["methods"].items()}
    assert min(_s, key=_s.get) == "MOVER-R", _s
m("CorrMC", "%d" % r9["MC"])
_mr = [x["methods"]["MOVER-R"]["nondeg"]["conditional"][0] for x in _g]
m("CorrCondMRLo", f3(min(_mr))); m("CorrCondMRHi", f3(max(_mr)))
pr = J("r9_paired_reward.json")
assert pr["paired_mean_gain"][1] > 0
m("RJvRewGainCI", "%.1f [%.1f, %.1f]" % tuple(pr["paired_mean_gain"]))
...
def bold(v, best):
    """Bold a formatted cell when it equals the best value in a directional column."""
    return "\\textbf{%s}" % v if v == best else v


lines = ["% generated by tools/make_numbers.py", "\\begin{tabular}{@{}lccccccccccc@{}}", "\\toprule",
         " & \\multicolumn{2}{c}{degenerate share} & \\multicolumn{2}{c}{non-degenerate acc.} & \\multicolumn{2}{c}{bounded acc.} & & & & \\\\",
         "\\cmidrule(lr){2-3}\\cmidrule(lr){4-5}\\cmidrule(lr){6-7}",
         "method & point & unbnd. & report & cond. & report & cond. & joint & set & common-F & med.\\ width & $S_{1/2}$ \\\\", "\\midrule"]
RECOMMENDED = "MOVER-R"
for meth, sh in METH + NEW_METH:
    b = A["methods"][meth]
    if meth == "Fieller":
        lines.append("\\multicolumn{12}{@{}l@{}}{\\emph{pre-registered in the frozen manifest}} \\\\")
    if meth == "Bayes":
        lines.append("\\multicolumn{12}{@{}l@{}}{\\emph{added after the manifest, exploratory}} \\\\")
    cells = [meth, f3(b["kinds"]["point"]), f3(b["kinds"]["line"]),
             f3(b["nondeg"]["report"][0]), f3(b["nondeg"]["conditional"][0]),
             f3(b["bounded"]["report"][0]), f3(b["bounded"]["conditional"][0]),
             f3(b["nondeg"]["joint"][0]), f3(b["set_coverage"][0]),
             f3(b["common_F"][0]), f3(b["nondeg"]["width_median"][0]),
             f3(b["score"]["nondeg"]["0.50"][0])]
    if meth == RECOMMENDED:
...
POL_ROWS = (("Uni", "uniform"), ("BvOne", "UCB-BV1"), ("BvTwo", "UCB-BV2"), ("Kube", "KUBE"), ("Ts", "Thompson on ratio"),
            ...
            ("Jv", "UCB-F, delta-JV fallback"), ("Inf", "UCB-F, $+\\infty$ fallback"))
...
_pol = [(disp, f3(run(t)["zero_outcome_frac"][0]), f3(run(t)["methods"]["Fieller"]["report"][0]),
         f3(run(t)["methods"]["Fieller"]["conditional"][0]), "%.0f" % run(t)["reward_mean"][0]) for t, disp in POL_ROWS]
_pol += [(disp, f3(r5[k]["zero_outcome_frac"][0]), f3(r5[k]["methods"]["Fieller"]["report"][0]),
          f3(r5[k]["methods"]["Fieller"]["conditional"][0]), "%.0f" % r5[k]["reward"][0]) for k, disp in R5_ROWS]
_best_rep2 = max(x[2] for x in _pol)
_best_rew2 = "%.0f" % max(float(x[4]) for x in _pol)
_HEADS = {"uniform": "\\emph{budget-oblivious control}",
          "UCB-BV1": "\\emph{budgeted bandit indices}",
          "UCB-F, delta fallback": "\\emph{Fieller index with a fallback rule}",
          "KL-UCB": "\\emph{regularised indices}"}
OURS = "UCB-F, delta-JV fallback"
for disp, zo, rep, cond, rew in _pol:
    if disp in _HEADS:
        lines.append("\\multicolumn{5}{@{}l@{}}{%s} \\\\" % _HEADS[disp])
    cells = [disp, zo, rep, cond, rew]
    if disp == OURS:
        cells = ["\\textbf{%s}" % c for c in cells]
    lines.append(" & ".join(cells) + " \\\\")
lines += ["\\bottomrule", "\\end{tabular}"]
NPOL = len(POL_ROWS) + len(R5_ROWS)
_pz = [run(t)["zero_outcome_frac"][0] for t, _ in POL_ROWS] + [r5[k]["zero_outcome_frac"][0] for k, _ in R5_ROWS]
_pznz = [v for v in _pz if v > 0]
open(os.path.join(ROOT, "numbers.tex"), "a", encoding="utf-8").write(
    "\\newcommand{\\NPol}{%s}\n\\newcommand{\\NPolNum}{%d}\n"
    "\\newcommand{\\PolZeroLo}{%s}\n\\newcommand{\\PolZeroHi}{%s}\n\\newcommand{\\PolZeroLoNz}{%s}\n" % (
        {12: "twelve", 13: "thirteen", 14: "fourteen", 15: "fifteen"}.get(NPOL, str(NPOL)), NPOL,
        f3(min(_pz)), f3(max(_pz)), f3(min(_pznz))))
open(os.path.join(ROOT, "table_policy.tex"), "w", encoding="utf-8").write("\n".join(lines) + "\n")


def row_sel(disp, b, mark=False):
    cells = [disp, f3(b["report"][0]), ci3s(b["conditional"]), f3(b["joint"][0]), f3(b["width_median"])]
    if mark:
        cells = ["\\textbf{%s}" % c for c in cells]
    return " & ".join(cells) + " \\\\"
...
lines = ["% generated by tools/make_numbers.py", "\\begin{tabular}{@{}lcccc@{}}", "\\toprule",
         "method & report & conditional & joint & med.\\ width \\\\", "\\midrule"]
...
for disp, key in (("Fieller", "Fieller|naive"), ("Fieller, $\\alpha/K$", "Fieller|bonferroni"), ("Fieller, split", "Fieller|split")):
    lines.append(row_sel(disp, SA[key], mark="bonferroni" in key))
